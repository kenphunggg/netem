...
def unset(config: Config):
    ...
    print(f"reset config for {config.interface}")
...
def config_full(config: Config):
    ...
    for index, host in enumerate(config.hosts):
        os.system(
            f"sudo tc qdisc add dev {config.interface} parent 1:{index+1} handle {unique_id}: htb"
        )
        os.system(
            f"sudo tc class add dev {config.interface} parent {unique_id}: classid {unique_id}:1 htb rate {host.bandwidth}kbit"
        )
        os.system(
            f"sudo tc qdisc add dev {config.interface} parent {unique_id}:1 handle {unique_id+1}: netem delay {host.latency}ms {host.jitter}ms"
        )
        os.system(
            f"sudo tc filter add dev {config.interface} protocol ip parent 1: prio {index+1} u32 match ip dst {host.ip_address}/32 flowid 1:{index+1}"
        )
        os.system(
...
#     os.system(
#         f"sudo tc qdisc add dev {host.interface} parent 1:1 handle 10: netem delay {host.latency}ms"
#     )
#     os.system(
#         f"sudo tc filter add dev {host.interface} protocol ip parent 1:0 prio 1 u32 match ip dst {host.ip_address}/32 flowid 1:1"
#     )
...
#     os.system(f"sudo tc qdisc add dev {host.interface} root handle 1: prio")
#     os.system(
#         f"sudo tc qdisc add dev {host.interface} parent 1:1 handle 10: netem delay {host.latency}ms {host.jitter}ms"
#     )
#     os.system(
#         f"sudo tc filter add dev {host.interface} protocol ip parent 1:0 prio 1 u32 match ip dst {host.ip_address}/32 flowid 1:1"
#     )
#     print(
#         f"Latency to {host.ip_address} set to {host.latency}ms with jitter {host.jitter}ms"
#     )
